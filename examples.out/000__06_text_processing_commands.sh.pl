#!/usr/bin/env perl
use strict;
use warnings;
use Carp;
use English qw(-no_match_vars);
use locale;
select((select(STDOUT), $| = 1)[0]);
use IPC::Open3;
use File::Path qw(make_path remove_tree);

my $main_exit_code = 0;
my $ls_success     = 0;
our $CHILD_ERROR;

my $MAGIC_5 = 5;
my $MAGIC_3 = 3;

print "=== Text Processing Commands ===\n";
my $file_content = do {
    my $output_127;
    my $pipeline_success_127 = 1;
    do { my $cat_cmd = 'cat src/main.rs'; $output_127 = qx{$cat_cmd}; };
    my $num_lines       = 5;
    my $head_line_count = 0;
    my $result          = q{};
    my $input           = $output_127;
    my $pos             = 0;

    while ( $pos < length $input && $head_line_count < $num_lines ) {
        my $line_end = index $input, "\n", $pos;
        if ( $line_end == -1 ) {
            $line_end = length $input;
        }
        my $head_line = substr $input, $pos, $line_end - $pos;
        $result .= $head_line . "\n";
        $pos = $line_end + 1;
        ++$head_line_count;
    }
    $output_127 = $result;

    if ( !$pipeline_success_127 ) { $main_exit_code = 1; }
    $output_127 =~ s/\n+\z//msx;
    $output_127;
};
print "First 5 lines of main.rs:\n";
print $file_content;
if ( !( $file_content =~ m{\n\z}msx ) ) { print "\n"; }
my $grep_result = do { my $grep_result_128;
my @grep_lines_128 = ();
my @grep_filenames_128 = ();
if (-e "src/main.rs") {
    open my $fh, '<', "src/main.rs" or croak "Cannot open file: $ERRNO";
    while (my $line = <$fh>) {
        chomp $line;
        push @grep_lines_128, $line;
        push @grep_filenames_128, "src/main.rs";
    }
    close $fh
        or croak "Close failed: $OS_ERROR";
}
else { print STDERR "grep: src/main.rs: No such file or directory\n"; }
my @grep_filtered_128 = grep { /fn/msx } @grep_lines_128;
my @grep_numbered_128;
for my $i (0..@grep_lines_128-1) {
    if (scalar grep { $_ eq $grep_lines_128[$i] } @grep_filtered_128) {
        push @grep_numbered_128, sprintf "%d:%s", $i + 1, $grep_lines_128[$i];
    }
}
$grep_result_128 = join "\n", @grep_numbered_128;
$CHILD_ERROR = scalar @grep_filtered_128 > 0 ? 0 : 1;
 $grep_result_128; };
print "Lines containing 'fn':\n";
print $grep_result;
if ( !( $grep_result =~ m{\n\z}msx ) ) { print "\n"; }
my $sed_result = do {
    my $output_129;
    my $pipeline_success_129 = 1;
    $output_129 .= "Hello World\n";
    if ( !($output_129 =~ m{\n\z}msx) ) { $output_129 .= "\n"; }
    my @sed_lines_129 = split /\n/msx, $output_129;
    my @sed_result_129;
    foreach my $line (@sed_lines_129) {
    chomp $line;
    $line =~ s/World/Universe/gmsx;
    push @sed_result_129, $line;
    }
    $output_129 = join "\n", @sed_result_129;

    if ( !$pipeline_success_129 ) { $main_exit_code = 1; }
    $output_129 =~ s/\n+\z//msx;
    $output_129;
};
do {
    my $output = "Sed result: $sed_result";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
};
my $awk_result = do {
    my $output_130;
    my $pipeline_success_130 = 1;
    $output_130 .= "1 2 3 4 5\n";
    if ( !($output_130 =~ m{\n\z}msx) ) { $output_130 .= "\n"; }
    my @lines = split /\n/msx, $output_130;
    my @result;
    foreach my $line (@lines) {
    chomp $line;
    if ($line =~ /^\\s*$/msx) { next; }
    my @fields = split /\s+/msx, $line;
    if (@fields > 0) {
    my $sum = $fields[0] + $fields[1];
    push @result, $sum;
    }
    }
    $output_130 = join "\n", @result;

    if ( !$pipeline_success_130 ) { $main_exit_code = 1; }
    $output_130 =~ s/\n+\z//msx;
    $output_130;
};
do {
    my $output = "Awk sum result: $awk_result";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
};
my $sort_result = do {
    my $output_131;
    my $pipeline_success_131 = 1;
    $output_131 .= "zebra\napple\nbanana";
    if ( !($output_131 =~ m{\n\z}msx) ) { $output_131 .= "\n"; }
    my @sort_lines_131_1 = split /\n/msx, $output_131;
    my @sort_sorted_131_1 = sort @sort_lines_131_1;
    $output_131 = join "\n", @sort_sorted_131_1;
        if ($output_131 ne q{} && !($output_131 =~ m{\n\z}msx)) {
            $output_131 .= "\n";
        }
    if ( !$pipeline_success_131 ) { $main_exit_code = 1; }
    $output_131 =~ s/\n+\z//msx;
    $output_131;
};
print "Sorted words:\n";
print $sort_result;
if ( !( $sort_result =~ m{\n\z}msx ) ) { print "\n"; }
my $uniq_result = do {
    my $output_132;
    my $pipeline_success_132 = 1;
    $output_132 .= "apple\napple\nbanana\nbanana\ncherry";
    if ( !($output_132 =~ m{\n\z}msx) ) { $output_132 .= "\n"; }
    my @uniq_lines_132_1 = split /\n/msx, $output_132;
    @uniq_lines_132_1 = grep { $_ ne q{} } @uniq_lines_132_1; # Filter out empty lines
    my %uniq_seen_132_1;
    my @uniq_result_132_1;
    foreach my $line (@uniq_lines_132_1) {
    if (!$uniq_seen_132_1{$line}++) { push @uniq_result_132_1, $line; }
    }
    $output_132 = join "\n", @uniq_result_132_1;
        if ($output_132 ne q{} && !($output_132 =~ m{\n\z}msx)) {
            $output_132 .= "\n";
        }
    if ( !$pipeline_success_132 ) { $main_exit_code = 1; }
    $output_132 =~ s/\n+\z//msx;
    $output_132;
};
print "Unique words:\n";
print $uniq_result;
if ( !( $uniq_result =~ m{\n\z}msx ) ) { print "\n"; }
my $word_count = do {
    my $output_133;
    my $pipeline_success_133 = 1;
    $output_133 .= "Hello World\n";
    if ( !($output_133 =~ m{\n\z}msx) ) { $output_133 .= "\n"; }
    use IPC::Open3;
    my @wc_args_133_1 = ("-w");
    my ($wc_in_133_1, $wc_out_133_1, $wc_err_133_1);
    my $wc_pid_133_1 = open3($wc_in_133_1, $wc_out_133_1, $wc_err_133_1, 'wc', @wc_args_133_1);
    print {$wc_in_133_1} $output_133;
    close $wc_in_133_1 or die "Close failed: $!\n";
    $output_133 = do { local $/ = undef; <$wc_out_133_1> };
    close $wc_out_133_1 or die "Close failed: $!\n";
    waitpid $wc_pid_133_1, 0;
    if ( !$pipeline_success_133 ) { $main_exit_code = 1; }
    $output_133 =~ s/\n+\z//msx;
    $output_133;
};
my $line_count = do {
    my $output_134;
    my $pipeline_success_134 = 1;
    $output_134 .= "line1\nline2\nline3";
    if ( !($output_134 =~ m{\n\z}msx) ) { $output_134 .= "\n"; }
    use IPC::Open3;
    my @wc_args_134_1 = ("-l");
    my ($wc_in_134_1, $wc_out_134_1, $wc_err_134_1);
    my $wc_pid_134_1 = open3($wc_in_134_1, $wc_out_134_1, $wc_err_134_1, 'wc', @wc_args_134_1);
    print {$wc_in_134_1} $output_134;
    close $wc_in_134_1 or die "Close failed: $!\n";
    $output_134 = do { local $/ = undef; <$wc_out_134_1> };
    close $wc_out_134_1 or die "Close failed: $!\n";
    waitpid $wc_pid_134_1, 0;
    if ( !$pipeline_success_134 ) { $main_exit_code = 1; }
    $output_134 =~ s/\n+\z//msx;
    $output_134;
};
do {
    my $output = "Word count: $word_count";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
};
do {
    my $output = "Line count: $line_count";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
};
my $head_result = do {
    my $seq_output_136 = do {
    my $result = q{};
    for my $i (1..10) {
        $result .= "$i\n";
    }
    $result;
};
    my @seq_lines_136 = split /\n/msx, $seq_output_136;
    my $output_136 = q{};
    my $head_line_count = 0;
    foreach my $line (@seq_lines_136) {
        chomp $line;
        if ($head_line_count < 3) {
    if ($head_line_count > 0) { $output_136 .= "\n"; }
    $output_136 .= $line;
    ++$head_line_count;
} else {
    $line = q{}; # Clear line to prevent printing
    last; # Break out of the yes loop when head limit is reached
}
    }
    $output_136 =~ s/\n+\z//msx;
    $output_136;
};
do {
    my $output = "First 3 numbers: $head_result";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
};
my $tail_result = do {
    my $seq_output_138 = do {
    my $result = q{};
    for my $i (1..10) {
        $result .= "$i\n";
    }
    $result;
};
    my @seq_lines_138 = split /\n/msx, $seq_output_138;
    my $output_138 = q{};
    my @tail_lines = ();
    foreach my $line (@seq_lines_138) {
        chomp $line;
        # tail -3: collecting all lines first (pipeline limitation)
        push @tail_lines, $line;
        $line = q{}; # Clear line to prevent printing
    }
    if (@tail_lines > 0) {
        my @last_lines = @tail_lines[-3..-1];
        $output_138 = join "\n", @last_lines;
        if ($output_138 ne q{}) {
            $output_138 .= "\n";
        }
    }
    $output_138 =~ s/\n+\z//msx;
    $output_138;
};
do {
    my $output = "Last 3 numbers: $tail_result";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
};
my $cut_result = do {
    my $output_139;
    my $pipeline_success_139 = 1;
    $output_139 .= "apple:banana:cherry\n";
    if ( !($output_139 =~ m{\n\z}msx) ) { $output_139 .= "\n"; }
    my @lines_140 = split /\n/msx, $output_139;
    my @result_140;
    foreach my $line (@lines_140) {
    chomp $line;
    my @fields = split /\:/msx, $line;
    if (@fields > 1) {
    push @result_140, $fields[1];
    }
    }
    $output_139 = join "\n", @result_140;

    if ( !$pipeline_success_139 ) { $main_exit_code = 1; }
    $output_139 =~ s/\n+\z//msx;
    $output_139;
};
do {
    my $output = "Second field: $cut_result";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
};
do {
    open my $original_stdout, '>&', STDOUT
      or die "Cannot save STDOUT: $!\n";
    open STDOUT, '>', 'temp1.txt'
      or die "Cannot open file: $!\n";
    print "1
2
3\n";
    open STDOUT, '>&', $original_stdout
      or die "Cannot restore STDOUT: $!\n";
    close $original_stdout
      or die "Close failed: $!\n";
};
do {
    open my $original_stdout, '>&', STDOUT
      or die "Cannot save STDOUT: $!\n";
    open STDOUT, '>', 'temp2.txt'
      or die "Cannot open file: $!\n";
    print "a
b
c\n";
    open STDOUT, '>&', $original_stdout
      or die "Cannot restore STDOUT: $!\n";
    close $original_stdout
      or die "Close failed: $!\n";
};
my $paste_result = do {
    my $output_141;
    my $pipeline_success_141 = 1;
    $output_141 = do {
        my @paste_file1_lines_fh_1;
        my @paste_file2_lines_fh_1;
        if (open my $fh1, '<', 'temp1.txt') {
            while (my $line = <$fh1>) {
                chomp $line;
                push @paste_file1_lines_fh_1, $line;
            }
            close $fh1 or croak "Close failed: $OS_ERROR";
        }
        if (open my $fh2, '<', 'temp2.txt') {
            while (my $line = <$fh2>) {
                chomp $line;
                push @paste_file2_lines_fh_1, $line;
            }
            close $fh2 or croak "Close failed: $OS_ERROR";
        }
        my $max_lines = scalar @paste_file1_lines_fh_1 > scalar @paste_file2_lines_fh_1 ? scalar @paste_file1_lines_fh_1 : scalar @paste_file2_lines_fh_1;
        my $paste_output = q{};
        for my $i (0..$max_lines-1) {
            my $line1 = $i < scalar @paste_file1_lines_fh_1 ? $paste_file1_lines_fh_1[$i] : q{};
            my $line2 = $i < scalar @paste_file2_lines_fh_1 ? $paste_file2_lines_fh_1[$i] : q{};
            $paste_output .= "$line1\t$line2\n";
        }
        $paste_output
        };
    my @sed_lines_141 = split /\n/msx, $output_141;
    my @sed_result_141;
    foreach my $line (@sed_lines_141) {
    chomp $line;
    $line =~ s/\t/ /gmsx;
    push @sed_result_141, $line;
    }
    $output_141 = join "\n", @sed_result_141;

    if ( !$pipeline_success_141 ) { $main_exit_code = 1; }
    $output_141 =~ s/\n+\z//msx;
    $output_141;
};
print "Pasted columns:\n";
print $paste_result;
if ( !( $paste_result =~ m{\n\z}msx ) ) { print "\n"; }
if ( -e "temp1.txt" ) {
    if ( -d "temp1.txt" ) {
        carp "rm: carping: ", "temp1.txt",
          " is a directory (use -r to remove recursively)\n";
    }
    else {
        if ( unlink "temp1.txt" ) {
            $main_exit_code = 0;
        }
        else {
            carp "rm: carping: could not remove ", "temp1.txt",
              ": $OS_ERROR\n";
        }
    }
}
else {
    local $CHILD_ERROR = 0;
    carp "rm: carping: ", "temp1.txt", ": No such file or directory\n";
}
if ( -e "temp2.txt" ) {
    if ( -d "temp2.txt" ) {
        carp "rm: carping: ", "temp2.txt",
          " is a directory (use -r to remove recursively)\n";
    }
    else {
        if ( unlink "temp2.txt" ) {
            $main_exit_code = 0;
        }
        else {
            carp "rm: carping: could not remove ", "temp2.txt",
              ": $OS_ERROR\n";
        }
    }
}
else {
    local $CHILD_ERROR = 0;
    carp "rm: carping: ", "temp2.txt", ": No such file or directory\n";
}
do {
    open my $original_stdout, '>&', STDOUT
      or die "Cannot save STDOUT: $!\n";
    open STDOUT, '>', 'file1.txt'
      or die "Cannot open file: $!\n";
    print "apple
banana
cherry\n";
    open STDOUT, '>&', $original_stdout
      or die "Cannot restore STDOUT: $!\n";
    close $original_stdout
      or die "Close failed: $!\n";
};
do {
    open my $original_stdout, '>&', STDOUT
      or die "Cannot save STDOUT: $!\n";
    open STDOUT, '>', 'file2.txt'
      or die "Cannot open file: $!\n";
    print "banana
cherry
date\n";
    open STDOUT, '>&', $original_stdout
      or die "Cannot restore STDOUT: $!\n";
    close $original_stdout
      or die "Close failed: $!\n";
};
my $comm_result = do { my @file1_lines;
my @file2_lines;
if (open my $fh1, '<', 'file1.txt') {
    while (my $line = <$fh1>) {
        chomp $line;
        push @file1_lines, $line;
    }
    close $fh1 or croak "Close failed: $OS_ERROR";
}
if (open my $fh2, '<', 'file2.txt') {
    while (my $line = <$fh2>) {
        chomp $line;
        push @file2_lines, $line;
    }
    close $fh2 or croak "Close failed: $OS_ERROR";
}
my %file1_set = map { $_ => 1 } @file1_lines;
my %file2_set = map { $_ => 1 } @file2_lines;
my @common_lines;
foreach my $line (@file1_lines) {
    if (exists $file2_set{$line}) {
        push @common_lines, $line;
    }
}
my $comm_output = q{};
foreach my $line (@common_lines) {
    $comm_output .= $line . "\n";
}
$comm_output =~ s/\n$//msx;
$comm_output };
print "Common lines:\n";
print $comm_result;
if ( !( $comm_result =~ m{\n\z}msx ) ) { print "\n"; }
my $diff_result = do { my $diff_exit_code = 0;
my $diff_output = q{};
{
    my $diff_cmd = 'diff';
    my @diff_args = ('file1.txt', 'file2.txt');
    my $diff_pid = open my $diff_fh, q{-|}, $diff_cmd, @diff_args;
    if ($diff_pid) {
        local $INPUT_RECORD_SEPARATOR = undef;
        $diff_output = <$diff_fh>;
        my $close_result = close $diff_fh; # Capture but ignore close result for diff
        $diff_exit_code = $CHILD_ERROR >> 8;
    } else {
        carp "Cannot execute diff command: $OS_ERROR";
        $diff_output = q{};
        $diff_exit_code = 1;
    }
}
$diff_output;
 };
print "File differences:\n";
print $diff_result;
if ( !( $diff_result =~ m{\n\z}msx ) ) { print "\n"; }
my $tr_result = do {
    my $_chomp_result = do {
    my $input_data = ("HELLO WORLD");
    my $set1_143 = 'A-Z';
my $set2_143 = 'a-z';
my $input_143 = $input_data;
# Expand character ranges for tr command
my $expanded_set1_143 = $set1_143;
my $expanded_set2_143 = $set2_143;
# Handle a-z range in set1
if ($expanded_set1_143 =~ /a-z/msx) {
    $expanded_set1_143 =~ s/a-z/abcdefghijklmnopqrstuvwxyz/msx;
}
# Handle A-Z range in set1
if ($expanded_set1_143 =~ /A-Z/msx) {
    $expanded_set1_143 =~ s/A-Z/ABCDEFGHIJKLMNOPQRSTUVWXYZ/msx;
}
# Handle a-z range in set2
if ($expanded_set2_143 =~ /a-z/msx) {
    $expanded_set2_143 =~ s/a-z/abcdefghijklmnopqrstuvwxyz/msx;
}
# Handle A-Z range in set2
if ($expanded_set2_143 =~ /A-Z/msx) {
    $expanded_set2_143 =~ s/A-Z/ABCDEFGHIJKLMNOPQRSTUVWXYZ/msx;
}
my $tr_result_142 = q{};
for my $char ( split //msx, $input_143 ) {
    my $pos_143 = index $expanded_set1_143, $char;
    if ( $pos_143 >= 0 && $pos_143 < length $expanded_set2_143 ) {
        $tr_result_142 .= substr $expanded_set2_143, $pos_143, 1;
    } else {
        $tr_result_142 .= $char;
    }
}
$tr_result_142
};
    chomp $_chomp_result;
    $_chomp_result;
};
do {
    my $output = "Lowercase: $tr_result";
    print $output;
    if ( !( $output =~ m{\n\z}msx ) ) {
        print "\n";
    }
};
my $xargs_result = do {
    my $output_144;
    my $pipeline_success_144 = 1;
    $output_144 .= "1 2 3\n";
    if ( !($output_144 =~ m{\n\z}msx) ) { $output_144 .= "\n"; }
    my @xargs_input_144_1 = split /\s+/msx, $output_144;
    my @xargs_output_144_1;
    for my $i (0..scalar @xargs_input_144_1-1) {
        my @xargs_args_144_1;
        for my $j (0..1-1) {
            push @xargs_args_144_1, $xargs_input_144_1[$i + $j];
        }
        my $xargs_line_144_1 = q{};
        $xargs_line_144_1 .= "Number:";
        foreach my $arg (@xargs_args_144_1) {
            $xargs_line_144_1 .= q{ } . $arg;
        }
        push @xargs_output_144_1, $xargs_line_144_1;
    }
    my $xargs_result_144_1 = join "\n", @xargs_output_144_1;
    $output_144 = $xargs_result_144_1;

    if ( !$pipeline_success_144 ) { $main_exit_code = 1; }
    $output_144 =~ s/\n+\z//msx;
    $output_144;
};
print "Xargs result:\n";
print $xargs_result;
if ( !( $xargs_result =~ m{\n\z}msx ) ) { print "\n"; }
if ( -e "file1.txt" ) {
    if ( -d "file1.txt" ) {
        carp "rm: carping: ", "file1.txt",
          " is a directory (use -r to remove recursively)\n";
    }
    else {
        if ( unlink "file1.txt" ) {
            $main_exit_code = 0;
        }
        else {
            carp "rm: carping: could not remove ", "file1.txt",
              ": $OS_ERROR\n";
        }
    }
}
else {
    local $CHILD_ERROR = 0;
    carp "rm: carping: ", "file1.txt", ": No such file or directory\n";
}
if ( -e "file2.txt" ) {
    if ( -d "file2.txt" ) {
        carp "rm: carping: ", "file2.txt",
          " is a directory (use -r to remove recursively)\n";
    }
    else {
        if ( unlink "file2.txt" ) {
            $main_exit_code = 0;
        }
        else {
            carp "rm: carping: could not remove ", "file2.txt",
              ": $OS_ERROR\n";
        }
    }
}
else {
    local $CHILD_ERROR = 0;
    carp "rm: carping: ", "file2.txt", ": No such file or directory\n";
}

exit $main_exit_code;
