    ...
    my $left_result_2 = do {
        ...
        my $eval_result = eval {
            ...
            if ( -e 'test_file_copy.txt' ) {
                ...
                if ( -e $dest && !$force ) {
                    croak
                      "mv: $dest: File exists (use -f to force overwrite)\n";
                }
                my $dest_dir = $dest;
                $dest_dir =~ s/\/[^\/]*$//msx;
                if ( $dest_dir eq $dest ) {
                ...
                if ( $dest_dir ne q{} && !-d $dest_dir ) {
                    ...
                    if ( @{$err} ) {
                        croak
                          "mv: cannot create directory $dest_dir: $err->[0]\n";
                    }
                }
...
if ( -e "test_dir/file" ) {
    if ( -d "test_dir/file" ) {
        ...
        if ( unlink "test_dir/file" ) {
            ...
            croak "rm: cannot remove ", "test_dir/file", ": $OS_ERROR\n";
        }
    }
}
else {
...
if ( -e "test_dir" ) {
    if ( -d "test_dir" ) {
        ...
        if ( unlink "test_dir" ) {
            ...
            carp "rm: carping: could not remove ", "test_dir", ": $OS_ERROR\n";
        }
    }
}
else {
