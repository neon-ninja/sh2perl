...
pub fn generate_sha256sum_command(generator: &mut Generator, cmd: &SimpleCommand, input_var: &str) -> String {
    ...
    if check_mode {
        ...
        output.push_str("my $pid = open3($in, $out, $err, 'sha256sum', \\\"$filename\\\");\n");
        ...
        output.push_str("\n");
        output.push_str("my ($in, $out, $err);\n");
        output.push_str(&format!("my $pid = open3($in, $out, $err, 'bash', '-c', 'echo -n \\\"${}\\\" | sha256sum');\n", input_var));
        output.push_str("close $in or croak 'Close failed: $!';\n");
        ...
        for file in &files {
            ...
            output.push_str("\n");
            output.push_str("my ($in, $out, $err);\n");
            output.push_str(&format!("my $pid = open3($in, $out, $err, 'sha256sum', {});\n", file));
            output.push_str("close $in or croak 'Close failed: $!';\n");
            output.push_str("my $hash = do { local $INPUT_RECORD_SEPARATOR = undef; <$out> };\n");
            output.push_str("close $out or croak 'Close failed: $!';\n");
            output.push_str("waitpid $pid, 0;\n");
