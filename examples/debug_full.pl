...
# Pipeline 2: cat file.txt | sort | uniq -c | sort -nr
my $output_2;
$output_2 = '';
...
print "DEBUG: output_2 after cat: length=" . length($output_2) . "\n";

my @sort_lines_2_1 = split(/\n/, $output_2);
print "DEBUG: sort_lines_2_1 count=" . scalar(@sort_lines_2_1) . "\n";
foreach my $i (0..$#sort_lines_2_1) {
    print "DEBUG: sort_lines_2_1[$i] = [" . $sort_lines_2_1[$i] . "]\n";
}

my @sort_sorted_2_1 = sort @sort_lines_2_1;
$output_2 = join("\n", @sort_sorted_2_1);

print "DEBUG: output_2 after sort: length=" . length($output_2) . "\n";

my @uniq_lines_2_2 = split(/\n/, $output_2);
print "DEBUG: uniq_lines_2_2 count before filter=" . scalar(@uniq_lines_2_2) . "\n";
foreach my $i (0..$#uniq_lines_2_2) {
    print "DEBUG: uniq_lines_2_2[$i] = [" . $uniq_lines_2_2[$i] . "]\n";
}

@uniq_lines_2_2 = grep { $_ ne '' } @uniq_lines_2_2; # Filter out empty lines
print "DEBUG: uniq_lines_2_2 count after filter=" . scalar(@uniq_lines_2_2) . "\n";
...
print "DEBUG: uniq result:\n";
foreach my $line (@uniq_result_2_2) {
    print "DEBUG: uniq result line: [$line]\n";
}
...
# Pipeline 3: find . -name "*.sh" | xargs grep -l "function"  | tr -d "\\\\/"
my $output_3;
my @find_files_4;
sub find_files_4 {
    my ($dir, $pattern) = @_;
...
print "DEBUG: find results count=" . scalar(@find_files_4) . "\n";
foreach my $i (0..$#find_files_4) {
    print "DEBUG: find_files_4[$i] = [" . $find_files_4[$i] . "]\n";
}
